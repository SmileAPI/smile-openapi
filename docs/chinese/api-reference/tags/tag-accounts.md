...
每个提供商支持的可用数据端点可能有所不同，您可以使用 [Developer Portal 下的 Data Sources 部分](https://portal.getsmileapi.com/providers?utm_source=docs&utm_medium=internal_link) 实时检查。您也可以使用 [Webhook 事件通知](/reference/chapter-5-cn) 跟踪数据的可用性。
